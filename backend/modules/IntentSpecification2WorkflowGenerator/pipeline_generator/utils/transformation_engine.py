...
def run_component_transformation(ontology: Graph, dataset:Dataset, component_transformations, inputs: List[URIRef],
                                 outputs: List[URIRef],
                                 parameters_specs: Dict[URIRef, Tuple[URIRef, Literal, Literal]]) -> None:
    data_graph = dataset.data_node_graph

    data_graph.serialize(f"./{inputs[0].fragment}_data.ttl")
