...
from ..simple.scaling import scaling_applier_implementation, scaling_learner_implementation, mode, min, max, cols, scailing_applier_output_data
from common import *
...
knime_normalizer_implementation = KnimeImplementation(
    ...
    parameters=[
        KnimeFactorParameter( 'mode', levels={1:"MinMax", 2:"ZScore", 3:"Decimal"}, base_parameter=mode, default_value=None, datatype=XSD.int),
        KnimeNumericParameter('newmin', XSD.float, expression=AlgebraicExpression(cb.COPY, min), default_value=0),
        KnimeNumericParameter('newmax', XSD.float, expression=AlgebraicExpression(cb.COPY, max), default_value=1.0),
...
knime_normalizer_applier_implementation = KnimeImplementation(
    ...
    output_ports=[
        cb.NONE,
        scailing_applier_output_data.get_uri(scaling_applier_implementation.uri_ref)
    ]
)